...
function findIndexByProperty(arr, name) {
  return arr.findIndex((value) => value.name === name);
...
function findLastEvenNumber(arr) {
  return arr.findLast((value) => value % 2 === 0) || "Парних чисел не знайдено";
...
console.log(findLastEvenNumber([1, 3, 5])); // Виведе 4
...
function getPositiveArrayValues(arr) {
  return Array.from(arr.values()).filter((value) => value > 0);
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function expandArrayByN(arr, n) {
  ...
  return arr.flatMap((value) => (value = [value, value, value]));
...
function findLongestWord(arr) {
  const result = arr.reduce((accum, value) => {
    return value.length > accum.length ? value : accum;
  }, "");
...
function capitalizeWords(arr) {
  return arr.map((value) => value.at(0).toUpperCase() + value.slice(1));
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо пустий масив
...
function calculateTotalPrice(arr) {
  return arr.reduce((accum, value) => accum + value.price, 0);
...
function findLastNegativeNumberIndex(arr) {
  return arr.findLastIndex((value) => value < 0);
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо -1
